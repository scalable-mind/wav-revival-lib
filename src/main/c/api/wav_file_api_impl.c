...
#endif
    if (strcmp(file_io_mode, "rb") == 0) {
        fread(&(wav_file->header), sizeof(WavHeader), 1, wav_file->file);
        wav_file->buffer_size *= wav_file->header.num_channels;
    }
    return wav_file;
}
...
static size_t write_next_chunk(WavFile* self, void* buffer, size_t size) {
    size_t sample_size = self->header.block_align / self->header.num_channels;
    return fwrite(buffer, sample_size, size, self->file);
}

static size_t write_header(WavFile* self, WavHeader* header) {
    self->header = *header;
    self->buffer_size *= header->num_channels;
